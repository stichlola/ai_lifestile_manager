                                ...
                                <div class="card h-100" :class="{ 'is-today': dayName === todayStr }">
                                    <div class="card-body">
                                        ...
                                        <div class="mt-3 small">
                                            <p v-for="(desc, name) in dayData.meal" :key="name" class="mb-2"><strong>{{ name }}:</strong> {{ desc }}</p>
                                        </div>
                                    </div>
                                    <div class="card-footer bg-transparent border-0 text-end">
                                        <button @click="openEditMealModal(dayName)" class="btn btn-outline-primary btn-square" style="width:40px;height:40px;display:inline-flex;align-items:center;justify-content:center;">
                        ...
                        <div class="row row-cols-1 row-cols-md-2 g-4">
                             <div v-for="(dayData, dayName) in plan" :key="`workout-${dayName}`" class="col">
                                <div class="card h-100" :class="{ 'is-today': dayName === todayStr }">
                                    ...
                                    <div class="card-footer bg-transparent border-0 text-end">
                                        <button @click="openEditWorkoutModal(dayName)" class="btn btn-outline-primary btn-square" style="width:40px;height:40px;display:inline-flex;align-items:center;justify-content:center;">
                                            <i class="material-icons">edit</i>
                                        </button>
                                    </div>
                                </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div v-if="editableMeals.length > 0">
                            <div v-for="(item, index) in editableMeals" :key="index" class="d-flex align-items-center mb-3">
                                <div class="form-outline flex-fill me-2">
                                    <input type="text" v-model="item.name" class="form-control" :id="`meal-name-${index}`" />
                                    <label class="form-label" :for="`meal-name-${index}`">Nome Pasto (es. Colazione 8:00)</label>
                                </div>
                                <div class="form-outline flex-fill me-2">
                                    <input type="text" v-model="item.desc" class="form-control" :id="`meal-desc-${index}`" />
                                    <label class="form-label" :for="`meal-desc-${index}`">Descrizione</label>
                                </div>
                                <button @click="removeMeal(index)" class="btn btn-link text-danger p-2"><i class="material-icons">delete</i></button>
                            </div>
                        </div>
                    ...
                    <div class="modal-body">
                        <div class="form-outline mb-3" v-if="editableWorkout">
                            <input type="text" v-model="editableWorkout.title" class="form-control" id="workout-title-edit"/>
                             <label class="form-label" for="workout-title-edit">Titolo Allenamento</label>
                        </div>
                        ...
                        <div v-if="editableWorkout">
                            <div v-for="(exercise, index) in editableWorkout.details" :key="index" class="p-3 border rounded mb-3">
                                <div class="form-outline mb-3">
                                    <input type="text" v-model="exercise.name" class="form-control" :id="`ex-name-${index}`" />
                                     <label class="form-label" :for="`ex-name-${index}`">Nome Esercizio</label>
                                </div>
                                <div class="input-group">
                                    <div class="form-outline flex-fill">
                                        <input type="url" v-model="exercise.link" class="form-control" :id="`ex-link-${index}`"/>
                                        <label class="form-label" :for="`ex-link-${index}`">Link YouTube (opzionale)</label>
                                    </div>
                                    <button @click="searchYoutube(exercise.name)" class="btn btn-outline-secondary" type="button">Cerca</button>
                                </div>
                                <div class="text-end mt-2">
                                    <button @click="removeExercise(index)" class="btn btn-link text-danger p-2"><i class="material-icons">delete</i></button>
                                </div>
                            </div>
                        </div>
          ...
          <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <div id="timer-wrapper" class="my-4">
                    <div id="collapsed-timer" class="text-center" @click="expandTimer">
                        <h6 class="text-muted mb-0 d-flex align-items-center justify-content-center"><i class="material-icons me-2">timer</i> Timer di Recupero</h6>
                    </div>
                    <div class="timer-container text-center">
                        <div class="timer-circle">
                            <svg width="120" height="120" viewBox="0 0 120 120">
                                <circle class="timer-bg" cx="60" cy="60" r="54"></circle>
                                <circle class="timer-progress" cx="60" cy="60" r="54" ref="timerProgressRingRef"></circle>
                            </svg>
                            <div ref="timerDisplayRef" class="timer-text">00:00</div>
                        </div>
                        <div class="timer-presets my-3">
                            <button class="btn btn-outline-primary btn-sm" @click="setTimer(30)">30s</button>
                            <button class="btn btn-outline-primary btn-sm" @click="setTimer(60)">60s</button>
                            <button class="btn btn-outline-primary btn-sm" @click="setTimer(90)">90s</button>
                            <button class="btn btn-outline-primary btn-sm" @click="setTimer(120)">120s</button>
                        </div>
                        <div class="timer-controls">
                            <button ref="timerToggleBtnRef" class="btn btn-primary btn-lg btn-floating" @click="toggleTimer"><i class="material-icons">play_arrow</i></button>
                            <button class="btn btn-secondary btn-lg btn-floating" @click="resetTimer()"><i class="material-icons">refresh</i></button>
                        </div>
                    </div>
                </div>
                <hr>
                                ...
                                <div class="card meal-detail-card">
                                    <div class="card-body d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <p class="card-text small text-muted mb-0">{{ meal.desc }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                  <div class="list-group list-group-flush">
                       <a href="#" v-for="(suggestion, index) in aiSuggestions" :key="index" @click.prevent="addSuggestedMeal(suggestion)" class="list-group-item list-group-item-action suggestion-item">
...
const defaultPlan = {
    "Lunedì": {
        meal: { 
            "Colazione (8:00)": "200g yogurt greco 0% + 30g avena + 10g noci",
            "Spuntino (10:30)": "1 mela",
            "Pranzo (13:00)": "150g riso integrale + 150g pollo + verdure + 10g olio EVO",
            "Spuntino (16:30)": "15g mandorle",
            "Cena (19:30)": "200g pesce bianco + verdure + 15g olio EVO"
        },
        workout: { 
    ...
    "Martedì": { 
        meal: {
            "Colazione (8:00)": "2 uova strapazzate + 1 fetta pane integrale",
            "Spuntino (10:30)": "1 yogurt 0%",
            "Pranzo (13:00)": "130g pasta integrale + 150g tonno al naturale + verdure + 10g olio EVO",
            "Spuntino (16:30)": "2 gallette con 10g burro arachidi",
            "Cena (19:30)": "180g salmone + verdure + 15g olio EVO"
        }, 
        workout: { 
    ...
    "Mercoledì": { 
        meal: {
            "Colazione (8:00)": "200ml latte scremato + 30g muesli",
            "Spuntino (10:30)": "1 banana piccola",
            "Pranzo (13:00)": "200g patate dolci + 150g tacchino + verdure + 15g olio EVO",
            "Spuntino (16:30)": "15g pistacchi",
            "Cena (19:30)": "180g pollo alla piastra + verdure"
        }, 
        workout: { 
    ...
    "Giovedì": { 
        meal: {
            "Colazione (8:00)": "200g yogurt greco 0% + 30g fiocchi d’avena + 10g semi di chia",
            "Spuntino (10:30)": "1 pera",
            "Pranzo (13:00)": "150g riso basmati + 150g merluzzo + verdure + 10g olio EVO",
            "Spuntino (16:30)": "1 barretta proteica leggera (100 kcal)",
            "Cena (19:30)": "200g manzo magro + verdure"
        }, 
        workout: { 
    ...
    "Venerdì": { 
        meal: {
            "Colazione (8:00)": "2 uova sode + 1 fetta pane integrale",
            "Spuntino (10:30)": "1 yogurt 0%",
            "Pranzo (13:00)": "130g pasta integrale + 150g sgombro + verdure + 10g olio EVO",
            "Spuntino (16:30)": "15g noci",
            "Cena (19:30)": "180g pollo o tacchino + verdure + 15g olio EVO (pasto libero sera se vuoi)"
        }, 
        workout: { 
    ...
    "Sabato": { 
        meal: {
            "Colazione (8:00)": "200g yogurt greco 0% + 30g muesli",
            "Spuntino (10:30)": "1 frutto di stagione",
            "Pranzo (13:00)": "200g patate + 150g pollo + verdure + 10g olio EVO",
            "Spuntino (16:30)": "2 gallette con 10g crema di arachidi",
            "Cena (19:30)": "200g salmone o pesce spada + verdure"
        }, 
        workout: { 
    ...
    "Domenica": { 
        meal: {
            "Colazione (8:00)": "200ml latte scremato + 30g avena + 10g mandorle",
            "Spuntino (10:30)": "1 mela o 1 yogurt",
            "Pranzo (13:00)": "150g riso integrale + 150g tacchino + verdure + 10g olio EVO",
            "Spuntino (16:30)": "15g nocciole",
            "Cena (19:30)": "200g pesce bianco + verdure + 15g olio EVO"
        }, 
        workout: { 
...
const nextMeal = computed(() => {
    if (!plan[todayStr.value] || !plan[todayStr.value].meal) {
        return { name: "Nessun piano per oggi", desc: "" };
    }
    const todayPlan = plan[todayStr.value];
    const now = new Date();
    const currentTime = now.getHours() * 60 + now.getMinutes();

    const mealsWithTime = Object.keys(todayPlan.meal)
        .map(name => {
            const match = name.match(/\((\d{1,2}):(\d{2})\)/);
            if (!match) return null;
            const time = parseInt(match[1]) * 60 + parseInt(match[2]);
            return { name, desc: todayPlan.meal[name], time };
        })
        .filter(Boolean)
    ...
    if (upcomingMeal) {
        return { name: `Prossimo Pasto: ${upcomingMeal.name}`, desc: upcomingMeal.desc };
    }
...
async function openEditMealModal(day) {
    ...
    editableMeals.value = Object.entries(plan[day].meal).map(([name, desc]) => ({ name, desc }));
...
function handleSaveMealChanges() {
    const newMealData = {};
    editableMeals.value.forEach(item => {
        if (item.name) newMealData[item.name] = item.desc;
    });
    plan[currentEditingDay.value].meal = newMealData;
    savePlan();
...
function addExercise() {
    if (editableWorkout.value) {
        editableWorkout.value.details.push({ name: 'Nuovo Esercizio', link: '' });
    }
}
...
function renderMealsModalContent(day) {
    ...
    if (mealPlan && mealPlan.meal) {
        Object.entries(mealPlan.meal).forEach(([name, desc]) => {
...
async function handleGenerateWorkout() {
    ...
    const geminiPrompt = `Sei un esperto personal trainer. Crea un piano di allenamento con 4 esercizi per "${workoutTitle}". Fornisci solo una risposta in formato JSON, seguendo questo schema: \`{"details": [{"name": "[Nome Esercizio 1 (serie x reps)]", "link": ""}]}\`. Non aggiungere testo o markdown.`;
...
async function handleSuggestMeals() {
    if (!checkAndUpdateAiUsage()) return;
    const mealType = window.prompt("Che tipo di pasto stai cercando? (es. colazione proteica, pranzo leggero)");
    if (!mealType) return;

    mdbLoadingModal.show();
    const geminiPrompt = `Sei un esperto nutrizionista. Suggerisci 3 idee per "${mealType}". Fornisci solo una risposta in formato JSON, seguendo questo schema: \`[{"name": "[Nome Pasto 1 (orario)]", "desc": "[Descrizione]"}]\`. Il nome del pasto deve includere un orario suggerito. Non aggiungere testo o markdown.`;
